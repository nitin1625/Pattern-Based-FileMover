...
class FileMoverApp:
    def __init__(self, root):
        ...
        self.create_ui()
        # Setup trace for source_path changes
        self.trace_id = self.source_path.trace_add('write', lambda *args: self.load_last_settings())  
    ...
    def create_ui(self):
        ...
        # Include Patterns
        include_frame = ttk.LabelFrame(main_frame, text=" Include Patterns (comma-separated) ", padding=10)
        include_frame.grid(row=4, column=0, sticky="ew", pady=5)
        
        ttk.Entry(include_frame, textvariable=self.include_patterns, width=60).pack(side=tk.LEFT, padx=5)
        ttk.Button(include_frame, text="?", width=3, command=self.show_pattern_help).pack(side=tk.LEFT)
...
list?.txt - files that start with 'list', are followed by 1 character and have exactly a .txt extension
    ...
    def load_config(self, config_path):
        ...
        try:
            with open(config_path, "r") as f:
                ...
                self.include_patterns.set(config.get("include_patterns", ""))
    ...
    def should_include(self, filepath):
        patterns = [p.strip() for p in self.include_patterns.get().split(',') if p.strip()]
    ...
    def preview_operation(self):
        ...
        try:
            ...
            for src_path in files:
                if  self.should_include(src_path):
                    ...
                    rel_src = os.path.relpath(src_path, source)
                    self.results_text.insert(tk.END, f"Will skip: {rel_src} (included)\n")
        except Exception as e:
